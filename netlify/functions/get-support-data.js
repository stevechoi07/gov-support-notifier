// v1.6: 백엔드 인메모리 캐싱 기능 추가
const fetch = require('node-fetch');

// --- 🧠 캐시 기억 장치 ---
// Netlify Function은 일정 시간 동안 살아있기 때문에, 이 변수에 데이터를 저장해 둘 수 있습니다.
const cache = {
  allData: null,
  timestamp: 0,
};
...
exports.handler = async function(event, context) {
  try {
    ...
    if (cache.allData && (now - cache.timestamp < CACHE_DURATION)) {
      console.log(`[v1.6 백엔드] ⚡️ 캐시 히트! (Cache Hit) 저장된 데이터를 사용합니다.`);
      // 캐시된 데이터를 사용해 필터링 및 페이지네이션 처리
      const filteredAndPaginatedData = processData(cache.allData, { page, perPage, searchTerm, region, category });
      return {
        statusCode: 200,
    ...
    console.log(`[v1.6 백엔드] 🐢 캐시 미스! (Cache Miss) 정부 API에 새로 요청합니다.`);
    ...
    console.log(`[v1.6 백엔드] 💾 새로운 데이터를 캐시에 저장합니다. (유효시간: 10분)`);
    cache.allData = allData;
    cache.timestamp = now;

    // 4. 새로 받아온 데이터를 사용해 필터링 및 페이지네이션 처리
    const filteredAndPaginatedData = processData(allData, { page, perPage, searchTerm, region, category });
    ...
    console.error('[v1.6 백엔드] 🔥 치명적 오류 발생:', error);
    return {
      statusCode: 500,
 ...
 */
function processData(allData, { page, perPage, searchTerm, region, category }) {
    console.log(`[v1.6 백엔드] ⚙️ 데이터 처리 시작...`, { page, perPage, searchTerm, region, category });
    ...
    console.log(`[v1.6 백엔드] ✅ 데이터 처리 완료! (필터링된 총 개수: ${totalItems}개, 현재 페이지에 보낼 개수: ${paginatedData.length}개)`);
